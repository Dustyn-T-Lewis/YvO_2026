...
source(here::here("04_Figures_v2", "F05", "a_script", "_panel_A_ORA.R"))
...
cfg <- list(
  ...
  QUAD_COLORS = c(
    "Reversed Up" = "#B2182B", "Reversed Down" = "#2166AC",
    "Non-reversed" = "#1B7837"
  ),
  QUAD_BG = c(
    "Reversed Up" = "#F4D9D2", "Reversed Down" = "#D5DEEF",
    "Non-reversed" = "#C8E0CD", "Tied" = "#EEEEEE"
  ),
  ENDPOINT_COLORS = c(
    "Reversed Up" = "#67001F", "Reversed Down" = "#053061",
    "Non-reversed" = "#00441B"
  ),
  SIG_COLORS = c(
    "Both" = "#2E7D32", "Aging" = "#E05A4E",
...
quad_legend <- ggplot(inset_quad_df) +
  geom_rect(
    aes(
      xmin = (as.integer(quadrant) - 1) * 3.5 +
  ...
  geom_rect(
    aes(
      xmin = (as.integer(quadrant) - 1) * 3.5 + 0.10 +
        (as.integer(quadrant) == 3) * nudge_idx3,
  ...
  geom_text(
    aes(
      x = (as.integer(quadrant) - 1) * 3.5 + 0.85 +
        (as.integer(quadrant) == 3) * nudge_idx3,
  ...
  theme(
    ...
    plot.margin = margin_auto(0, unit = "mm")
  )
...
composite <- composite + plot_annotation(theme = theme(plot.margin = margin_auto(-2.5, -1, unit = "mm")))
pC_fry <- pC_fry + plot_annotation(theme = theme(plot.margin = margin(3, 5, 0, 0, "mm")))
pD <- pD + theme(plot.margin = margin(-2.8, 5, 2.8, -5, "mm"))
pE_heat <- pE_heat + theme(
  plot.margin = margin(-2.1, -0.2, 3.4, -3.5, "mm"),
  axis.title = element_text(face = "bold", size = 8)
)
pB <- pB + coord_cartesian(
  xlim = c(-0.25, X_BAR_MAX + 1.75),
...
composite_final <- ggdraw(fig) +
  ...
  draw_label(
    "Panel D (NES reversal) uses Hallmark + GO Slim pathways; ORA panels use the full pathway collection.",
