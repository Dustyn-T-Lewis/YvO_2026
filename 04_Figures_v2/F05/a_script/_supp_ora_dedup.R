...
results <- vector("list", length(quad_set))
for (qi in seq_along(quad_set)) {
  quad <- quad_set[qi]
...
pS_ora_dedup <- ggplot(sens_df, aes(x = quadrant, y = n_enriched, fill = cutoff_label)) +
  ...
  labs(
    title = "ORA Dedup Sensitivity (Reversal Quadrants)",
    subtitle = sprintf(
      "Hypergeometric ORA | %d universe | Reversed quadrants only",
      length(universe)
    ),
