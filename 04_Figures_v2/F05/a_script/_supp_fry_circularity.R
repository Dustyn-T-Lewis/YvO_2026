...
sub_text <- sprintf(
  "Observed r = %.3f | Null mean = %.4f | p_perm = %.3f",
...
pS_circ <- ggplot(circ_df, aes(x = perm_r)) +
  ...
  geom_vline(
    xintercept = null_mean, color = "#4393C3", linewidth = 0.5,
    linetype = "dashed"
  ) +
  coord_cartesian(clip = "off") +
  annotate("label",
    x = obs_r, y = Inf, vjust = 1.3, hjust = 1.1,
    label = sprintf("Observed\nr = %.3f", obs_r),
    size = 2.5, fontface = "bold", color = "#D6604D",
    fill = alpha("white", 0.9), label.padding = unit(2, "pt")
  ) +
  annotate("label",
    x = null_mean, y = Inf, vjust = 1.3, hjust = -0.1,
    label = sprintf("Null mean\n= %.4f", null_mean),
    size = 2.2, fontface = "bold", color = "#4393C3",
    fill = alpha("white", 0.9), label.padding = unit(2, "pt")
  ) +
  labs(
    title = "Circularity Diagnostic: Protein-Permuted Null",
    subtitle = sub_text,
