...
make_dist_panel <- function(col, fill, xlab, vline = NULL, stat_fmt, title, tag) {
  ...
  if (col == "P.Value") {
    ...
    p <- p + geom_hline(
      data = uniform_ref, aes(yintercept = h),
  ...
  p <- p +
    ...
    FIG_THEME + theme_sub_strip(background = element_blank(), text = element_blank())
...
ma_df <- bind_rows(lapply(CTRS, \(ctr) {
  per_contrast[[ctr]] |>
    transmute(
      contrast = ctr, gene, average_intensity, logFC,
  ...
  mutate(
    contrast = factor(contrast, levels = CTRS),
...
pC <- ggplot(ma_df, aes(average_intensity, logFC, color = direction)) +
  ...
  geom_text(
    data = n_dep, aes(x = Inf, y = Inf, label = sprintf("\u03A0: %d", n_dep)),
  ...
  FIG_THEME +
  theme_sub_strip(background = element_blank(), text = element_blank()) +
  theme_sub_legend(position = "top", key.size = unit(3, "mm"))
...
if (!is.null(out_sens) && nrow(out_sens) > 0) {
  ...
  pE <- ggplot(long_df, aes(cohort, n, fill = cohort)) +
    ...
    scale_fill_manual(
      values = c(full = "#2166AC", reduced = "#B2182B"),
    ...
    theme_sub_strip(
      ...
      text.y = element_text(face = "bold", size = FIG_STRIP_SIZE - 1)
    ) +
    theme_sub_legend(position = "top", key.size = unit(3, "mm"))
} else {
  stop(
    "outlier_sensitivity sheet missing from 03_DEP_results.xlsx — ",
...
grid <- (pB | pPi) / (pFDR | pC) / pE &
  theme(plot.margin = margin_auto(9, 4, 4))
...
for (i in seq_along(all_tags)) {
  composite <- composite +
    draw_label(all_tags[i],
      x = all_xs[i], y = all_ys[i],
      fontface = "bold", size = txt$tag, hjust = 0, vjust = 1
    ) +
    draw_label(all_titles[[i]],
      x = all_xs[i] + X_TTL, y = all_ys[i],
