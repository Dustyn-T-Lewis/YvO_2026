...
if (!is.null(blunt)) {
  ...
  set.seed(42)
  ratio_ci <- quantile(
    replicate(2000, median(sample(lfc_to, replace = TRUE)) /
...
# Hoist candidate: pure-derived from CONTRAST_COLORS, could live in style.R.
...
frac_df <- bind_rows(lapply(CONTRASTS, \(ctr) {
  ...
  p_col <- paste0("P.Value_", ctr)
  tibble(
    contrast = SET_LABELS[ctr], threshold = c("p < 0.05", "q < 0.05", "Pi < 0.05"),
    n = c(
      sum(!is.na(dep_df[[p_col]]) & dep_df[[p_col]] < 0.05),
...
label_df <- frac_df |>
  ...
  mutate(
    next_pct = lead(pct, default = 0), seg_width = pct - next_pct,
...
pC <- ggplot(frac_df, aes(contrast, pct, fill = fill_key)) +
  ...
  # Separator flags Aging as the cross-sectional column, distinct in design
  # from the within-subject Training/Interaction contrasts below it.
  geom_vline(
    xintercept = 3.5, linetype = "dashed", color = "grey35", linewidth = 0.35
  ) +
  ...
  labs(
    ...
    subtitle = sprintf(
      "%s proteins | \u03A0 %d | FDR %d | p %d",
      format(n_total, big.mark = ","), pi_total, fdr_total, p_total
    ),
    caption = paste0(
      "Aging: cross-sectional (between-subject).\n",
      "Training/Interaction: within-subject (Pre→Post); not directly comparable."
    ),
...
pA <- pA + theme(plot.margin = margin_auto(12, 2))
# -52 right margin pulls B leftward to close the A/B gap; tuned to COMP_W = 178 mm.
...
# Title/subtitle baselines, resolved from the original nudge chains
# (tuned to COMP_H = 115 mm, ROW_TOP = 0.458, SPACER = 0).
TOP_Y <- 0.9856086956521739
BOT_Y <- 0.5401086956521739
X_LEFT <- 0.002
X_MID <- 0.372
X_RIGHT <- 0.630
X_MID_BOT <- X_MID
