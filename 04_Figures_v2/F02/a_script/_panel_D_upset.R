...
# SET_DISPLAY_COLORS is defined by the calling script (01_main_panels.R).
...
comb_deg_ord <- comb_degree(cm_sub)[comb_ord]
...
# Gray vertical divider between the single-contrast bars (intersection
# degree 1) and the multi-contrast intersection bars. Singles are ordered
# first, so the boundary sits after the last degree-1 bar. Not drawn when the
# display is all singles or all intersections.
n_single_bars <- sum(comb_deg_ord == 1)
divider_x <- if (n_single_bars > 0 && n_single_bars < n_int) {
  n_single_bars + 0.5
} else {
  NA_real_
}
...
if (nrow(sig_overlaps) > 0) {
  ...
  # Co-occurrence, not convergence: a shared protein differs at baseline by age
  # AND moves with training, rather than the two effects landing on one target.
  top_or_txt <- sprintf(
    "Strong %s<->%s co-occurrence (OR=%.1f)",
...
pD_bars <- ggplot(bar_long, aes(x, count, fill = direction)) +
  ...
  geom_text(
    data = \(d) d |> filter(count >= in_bar_min, is_single),
  ...
  geom_text(
    data = \(d) d |> filter(count > 0, count < in_bar_min, is_single),
    aes(
      x = x + ifelse(direction == "Up", 0.2, -0.2),
  ...
  geom_text(
    data = \(d) d |> filter(count == 0, is_single),
  ...
  labs(
    ...
    subtitle = sprintf(
      "%d unique \u03A0 DEPs | %d/%d co-occurring (FET)",
