...
# Reads frozen fGSEA cache from 04_Figures_v2/shared_functions/fgsea_tstat_all_v2.csv.
...
# Shared fGSEA cache — regenerated by 04_Figures_v2/shared_functions/F02-F03_build_fgsea_cache.R
...
DISPLAY_CONTRASTS <- CONTRAST_ORDER
...
pe_subtitle <- sprintf(
  "fGSEA | 5 databases | per-db BH | %d sig / %d tested",
  sum(count_df$count),
...
p <- p + labs(
  title = "Pathway Enrichment (Up/Down)", subtitle = pe_subtitle,
  caption = "Aging cross-sectional;\ntraining within-subject"
)
