...
results <- vector("list", length(target_quads))
for (qi in seq_along(target_quads)) {
  quad <- target_quads[qi]
  genes <- scatter_df$gene[scatter_df$quadrant == quad]
  if (length(genes) < 5) next

  quad_res <- vector("list", length(cutoffs))
  for (ci in seq_along(cutoffs)) {
    jc <- cutoffs[ci]
...
pS_ora_dedup <- ggplot(sens_df, aes(
  x = quadrant, y = n_enriched,
  fill = cutoff_label
)) +
  geom_col(
  ...
  scale_fill_manual(
    values = c(
      "J = 0.3" = "grey80", "J = 0.5" = "grey60",
      "J = 0.7" = "grey40", "J = 1.0" = "grey20"
    ),
    name = "Jaccard cutoff"
  ) +
  labs(
    title = "ORA Dedup Sensitivity",
