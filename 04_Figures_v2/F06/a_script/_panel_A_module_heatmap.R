...
mod_counts$bar_text_col <- ifelse(
  vapply(mod_counts$module_color, is_light_color, logical(1)),
  "black", "white"
)
...
build_count_bars <- function(txt_count, txt_cell) {
  ...
  ggplot(mod_counts, aes(x = x_sqrt, y = module)) +
    geom_col(
      fill = mod_counts$module_color, color = "black",
...
build_heatmap <- function(heat_df, col_labels, trait_order,
  ...
  p +
    ...
    geom_tile(
      data = heat_df |> filter(has_black),
      color = "black", linewidth = 1.0, fill = NA
    ) +
    geom_text(
      data = heat_df |> filter(!has_black & !is_large),
...
assemble_figure <- function(p_brackets, p_counts, p_heat,
  ...
  p + plot_annotation(
    ...
    subtitle = NULL,
    theme = theme(
      plot.margin = margin_auto(12, 2, 2) # extra top margin for stitcher-drawn title
    )
  )
}
...
shading <- tibble(
  xmin = c(
    xpos[1] - 0.5, xpos[5] - 0.5, xpos[7] - 0.5,
    xpos[9] - 0.5, xpos[12] - 0.5, xpos[15] - 0.5, xpos[17] - 0.5
  ),
  xmax = c(
    xpos[4] + 0.5, xpos[6] + 0.5, xpos[8] + 0.5,
