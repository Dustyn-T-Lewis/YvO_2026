...
# Trait matrix (age design + 6 phenotype). time_num and interaction are
# dropped from the pooled correlation: stacking Pre+Post gives 62 non-independent
# samples, and those two traits are the worst hit. Their inference lives on the
# LMM path (eigengene ~ group + (1|subject)) instead.
traits <- meta |>
  mutate(age_num = if_else(age == "Old", 1, 0))
...
trait_cols <- c("age_num", pheno_cols)
...
# Eigengene-trait Pearson r plus per-trait Student p (n = non-missing per trait).
cor_with_p <- function(me, traits) {
  cor_mat <- cor(me, traits, use = "pairwise.complete.obs")
  p_mat <- matrix(NA_real_,
    nrow = ncol(me), ncol = ncol(traits),
    dimnames = list(colnames(me), colnames(traits))
  )
  for (trait in colnames(traits)) {
    n_ok <- sum(!is.na(traits[, trait]))
    p_mat[, trait] <- corPvalueStudent(cor_mat[, trait], n_ok)
  }
  list(cor = cor_mat, p = p_mat)
}

mt <- cor_with_p(MEs, traits_mat)
module_trait_cor <- mt$cor
module_trait_pval <- mt$p
...
# Display-only: the pooled n=62 p-values are not subject-independent, so the
# heatmap shows correlations without significance stars.
text_matrix <- signif(module_trait_cor, 2)
...
plot_trait_heatmap <- function() {
  ...
  labeledHeatmap(
    ...
    cex.text = 0.5,
    zlim = c(-1, 1),
    main = "Module-trait correlations (pooled n=62 samples, not subject-independent; exploratory display only)"
  )
}
...
hub_rows <- lapply(unique_modules, function(mod) {
...
ora_results_list <- lapply(unique_modules, function(mod) {
...
bl <- cor_with_p(me_pre_raw[pre_subjects, all_mods, drop = FALSE], baseline_traits)
bl_cor_mat <- bl$cor
bl_pval_mat <- bl$p
...
ch <- cor_with_p(delta_me[common_subj, all_mods, drop = FALSE], change_traits)
ch_cor_mat <- ch$cor
ch_pval_mat <- ch$p
...
bl_y <- cor_with_p(me_pre_y, bl_traits_y)
bl_cor_young <- bl_y$cor
...
bl_o <- cor_with_p(me_pre_o, bl_traits_o)
bl_cor_old <- bl_o$cor
bl_pval_old <- bl_o$p
...
ch_y <- cor_with_p(dme_y, ch_traits_y)
ch_cor_young <- ch_y$cor
...
ch_o <- cor_with_p(dme_o, ch_traits_o)
ch_cor_old <- ch_o$cor
ch_pval_old <- ch_o$p
...
lmm_rows <- lapply(all_mods, function(mod) {
  ...
  contrast_rows <- lapply(names(lmm_contrast_list), function(cname) {
...
strat_rows <- lapply(c("Young", "Old"), function(age_grp) {
  strat_data <- lmm_data |>
  ...
  mod_rows <- lapply(all_mods, function(mod) {
...
# Per-module pick of the strongest BH-significant trait association across the
# subject-level sources: delta correlation (paired all/young/old) and the LMM
# Aging contrast. The pooled n=62 simple correlation is excluded — its p-values
# are not subject-independent. Falls back to age_num if nothing reaches BH<0.05.
