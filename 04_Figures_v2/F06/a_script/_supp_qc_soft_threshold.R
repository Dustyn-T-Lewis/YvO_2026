...
message(sprintf(
  "Soft threshold ggplot: loading cached fitIndices (%d powers, %d proteins)",
...
p1 <- ggplot(fit_df, aes(power, r2)) +
  geom_hline(
    yintercept = 0.85, linetype = "dashed", color = "grey40",
    linewidth = 0.4
  ) +
  geom_point(aes(fill = selected),
...
composite <- (p1 | p2) +
  plot_annotation(
    ...
    subtitle = sprintf(
      "Signed network | selected power = %d (R\u00b2 = %.3f) | %s proteins",
