...
# Headless runs (Rscript) drop a stray Rplots.pdf when a base-graphics call
# opens an implicit device. Redirect that device to a null sink; left active
# only in non-interactive sessions so RStudio plotting is untouched.
suppress_rplots_device <- function() {
  options(device = function(...) grDevices::pdf(file = nullfile(), ...))
}
if (!interactive()) suppress_rplots_device()
...
# Fisher z CI for a correlation (k = number of covariates). spearman = TRUE
# uses the Bonett & Wright 2000 variance (1 + r^2/2)/(n-3) for Spearman rho.
fisher_z_ci <- function(r, n, k = 0, level = 0.95, spearman = FALSE) {
  n_eff <- n - k
  if (n_eff < 4 || is.na(r)) {
    return(c(lo = NA_real_, hi = NA_real_))
  }
  z <- atanh(r)
  se <- if (spearman) sqrt((1 + r^2 / 2) / (n_eff - 3)) else 1 / sqrt(n_eff - 3)
...
boot_median_ci <- function(x, R = 2000, conf = 0.95, seed = 42) {
  old_seed <- if (exists(".Random.seed", .GlobalEnv)) get(".Random.seed", .GlobalEnv) else NULL
  set.seed(seed)
  on.exit(if (is.null(old_seed)) suppressWarnings(rm(".Random.seed", envir = .GlobalEnv)) else assign(".Random.seed", old_seed, .GlobalEnv))
