...
composite_final <- ggdraw(composite) +
  ...
  draw_label("A",
    x = TAG_X, y = TAG_Y_A - TAG_DY, size = TAG_SZ, fontface = "bold",
    hjust = 0, vjust = 1
  ) +
  draw_label(pA_title,
    x = TAG_X + X_TTL, y = TAG_Y_A,
    size = TTL_SZ, fontface = "bold", hjust = 0, vjust = 1
  ) +
  draw_label(pA_subtitle,
    x = TAG_X + X_TTL, y = TAG_Y_A - SUB_OFFSET,
    size = SUB_SZ, fontface = "bold.italic", colour = "grey40",
    hjust = 0, vjust = 1
  ) +
  # Panel B tag + title + subtitle
  draw_label("B",
    x = TAG_X, y = TAG_Y_B - TAG_DY, size = TAG_SZ, fontface = "bold",
    hjust = 0, vjust = 1
  ) +
  draw_label(pB_title,
    x = TAG_X + X_TTL, y = TAG_Y_B,
    size = TTL_SZ, fontface = "bold", hjust = 0, vjust = 1
  ) +
  draw_label(pB_subtitle,
    x = TAG_X + X_TTL, y = TAG_Y_B - SUB_OFFSET,
    size = SUB_SZ, fontface = "bold.italic", colour = "grey40",
    hjust = 0, vjust = 1
  )
...
f07_specs <- list(
  ...
  list(name = "exploratory_multivar_auc", path = f07("panel_A_multi_classifier_auc.csv")),
  list(name = "exploratory_multivar_featstab", path = f07("panel_A_feature_stability.csv")),
  list(name = "exploratory_multivar_perm", path = f07("panel_A_permutation.csv")),
  list(name = "exploratory_multivar_roc", path = f07("panel_A_roc_curves.csv")),
  list(name = "exploratory_pilot_me_roc", path = f07("classifier_pilot_summary.csv")),
  list(name = "exploratory_pilot_me_curves", path = f07("classifier_pilot_curves.csv")),
...
build_workbook(
  ...
  overview_df = data.frame(
    Sheet = c(
      "module_grid_summary",
      "module_grid_curves",
      "exploratory_multivar_auc",
      "exploratory_multivar_featstab",
      "exploratory_multivar_perm",
      "exploratory_multivar_roc",
      "exploratory_pilot_me_roc",
      "exploratory_pilot_me_curves",
      "panel_B_full_screen",
      "loso_auc_summary",
