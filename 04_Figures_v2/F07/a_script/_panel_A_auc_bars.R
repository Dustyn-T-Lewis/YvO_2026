...
#
# Reads : 04_Figures_v2/F07/c_data/module_grid/module_grid_{summary,curves}.csv
#         (generated by _supp_module_grid.R — must run that first)
# Writes: 04_Figures_v2/F07/b_reports/main/{png,pdf}/panels/MAIN_panel_A_auc.{png,pdf}
#
...
build_cell <- function(i) {
  ...
  # DeLong CI assumes independent observations, so it is invalid for the
  # paired Pre/Post rows; show it only where it is well defined.
  paired <- grepl("^Pre vs Post", rr$row)
  auc_line <- sprintf("AUC %.2f", rr$auc)
  ci_line <- if (paired) "CI n/a (paired)" else sprintf("[%.2f, %.2f]", rr$ci_lo, rr$ci_hi)
...
grid12 <- wrap_plots(cells, nrow = 2, ncol = 6) &
  theme(plot.margin = margin_auto(0, 0, 8))
...
build_key_cell <- function(mod, lab) {
  ggplot() +
    ...
    coord_cartesian(clip = "off") +
    theme_void() +
    theme(plot.margin = margin_auto(5, 1))
}
...
composite <- composite +
  plot_annotation(
    title = "In-sample per-module ROCs (top 12 of 48, ranked by permutation p)",
    subtitle = "In-sample (resubstitution) AUC: held-out LOSO controls in Supp C\u2013E. Top 12 of 48. Border: solid = BH q<0.05, dashed = nominal p<0.05.",
    theme = theme_sub_plot(
      title = element_text(
        face = "bold", size = FIG_TITLE_SIZE, color = "grey10",
        margin = margin(b = 1, l = 30)
      ),
      subtitle = element_text(
        size = FIG_SUBTITLE_SIZE, face = "italic", color = "grey30",
...
pA_title <- "In-sample per-module ROCs (top 12 of 48, ranked by permutation p)"
pA_subtitle <- "In-sample (resubstitution) AUC: held-out LOSO controls in Supp C\u2013E. Top 12 of 48. Border: solid = BH q<0.05, dashed = nominal p<0.05."
