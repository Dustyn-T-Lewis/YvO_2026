...
# Hero picks
# Exploratory, hypothesis-generating cells: VL/LBM outcomes crossed with the
# modules of interest. Hand-picked at raw p<0.10 in the Young stratum; none
# survive BH correction (see screen below). Each panel shows Young (solid) +
# Old (dashed) and is annotated with n and BH status.
HERO_PICKS <- tribble(
  ~x_source,   ~module,        ~outcome,
  "delta_ME",  "MEgreen",      "delta_VL", # OxPhos
  "baseline",  "MEturquoise",  "delta_VL", # Lipid Catabolism
  "delta_ME",  "MEyellow",     "delta_LBM", # Muscle Contraction
  "baseline",  "MEyellow",     "delta_VL" # Muscle Contraction
)
...
build_hero_mini <- function(x_source, module, outcome) {
  ...
  cor_by_age <- function(grp) {
    ...
    if (nrow(sub) < 4) {
      return(c(r = NA_real_, p = NA_real_, n = nrow(sub)))
    }
  ...
  # BH status for the Young stratum, pulled from the full screen (built below).
  pick_src <- x_source
  pick_mod <- mod
  pick_out <- outcome
  bh_row <- dplyr::filter(
    screen_df, source == pick_src, module == pick_mod,
    outcome == pick_out, stratum == "Young"
  )
  bh_stat <- if (nrow(bh_row) == 1) {
    sprintf("BH q=%.2f (%s)", bh_row$p_bh, if (bh_row$p_bh < 0.05) "sig" else "ns")
  } else {
    "BH q=n/a"
  }
  ...
  stat_label <- sprintf(
    paste0(
      ...
      "<span style='color:%s'><b>%s</b></span><br>",
      "<span style='color:grey40'>%s</span>"
    ),
    AGE_COLORS[["Young"]], y_stat, AGE_COLORS[["Old"]], o_stat, bh_stat
  )
...
age_screen <- subj_age$age[match(common_subj, subj_age$subject_key)]

screen_df <- bind_rows(lapply(SOURCES, function(src) {
  src_mat <- if (src == "baseline") me_pre else delta_me
  bind_rows(lapply(MODULES_SCREEN, function(mod) {
    x <- as.numeric(src_mat[common_subj, mod])
    bind_rows(lapply(OUTCOMES, function(out) {
      y <- pheno_wide[[out]][match(common_subj, pheno_wide$subject_key)]
      bind_rows(lapply(c("Young", "Old"), function(grp) {
        keep <- !is.na(x) & !is.na(y) & age_screen == grp
        if (sum(keep) < 4) {
          return(NULL)
        }
...
panel_B <- wrap_plots(hero_plots, ncol = GRID_NCOL) +
  plot_annotation(
    title = "Module\u2013phenotype coupling (exploratory, age-stratified)",
    subtitle = sprintf(
      "Hypothesis-generating: hand-picked at raw p<0.10; %d/%d raw p<0.05, %d/%d survive BH (not FDR-significant)",
...
pB_title <- "Module\u2013phenotype coupling (exploratory, age-stratified)"
pB_subtitle <- sprintf(
  "Hypothesis-generating: hand-picked at raw p<0.10; %d/%d raw p<0.05, %d/%d survive BH (not FDR-significant)",
  n_raw_sig, n_screen, n_bh_sig, n_screen
)
