...
# LOOCV engines (run_topk_loocv, fast_loocv_auc) live in _f07_helpers.R.
...
eval_clf <- function(name, labels, X, k_range, n_perm = 200) {
  ...
  list(
    name = name, auc = auc_v, ci_lo = ci[1], ci_hi = ci[3],
...
X_pheno <- pheno |>
  dplyr::filter(subject_key %in% common_subj) |>
...
resp_vl <- f07_loso_resid_median_split(pheno_subj$delta_VL, pheno_subj$age_bin)
resp_lbm <- f07_loso_resid_median_split(pheno_subj$delta_LBM, pheno_subj$age_bin)
...
results$resp_vl <- eval_clf("\u0394VL-resp|ME_Pre",
  resp_vl[ok4], as.matrix(me_pre[common_subj, ])[ok4, ],
  k_range = 2:5
)
...
results$resp_lbm <- eval_clf("\u0394LBM-resp|ME_Pre",
  resp_lbm[ok5], as.matrix(me_pre[common_subj, ])[ok5, ],
  k_range = 2:5
)
...
prot_feat <- prot_feat |>
  ...
  filter(
    !is.na(is_aging_dep), !is.na(mean_abund), !is.na(sd_abund),
    !is.na(module_color)
  )
mod_oh <- model.matrix(~ module_color - 1, data = prot_feat)
X_prot6 <- cbind(
  mean_abund = prot_feat$mean_abund,
...
fit_10fold_perm <- function(X, y, n_perm = 200, seed = 42) {
  ...
  for (f in 1:10) {
    ...
    fit <- tryCatch(suppressWarnings(glm(y ~ ., binomial,
      data = cbind(y = y[tr], as.data.frame(X[tr, ]))
    )), error = function(e) NULL)
    if (!is.null(fit)) {
      probs[!tr] <- predict(fit,
        type = "response",
  ...
  for (p in seq_len(n_perm)) {
    ys <- sample(y)
    pr <- numeric(length(ys))
    for (f in 1:10) {
      tr <- folds != f
      fit <- tryCatch(suppressWarnings(glm(y ~ ., binomial,
        data = cbind(y = ys[tr], as.data.frame(X[tr, ]))
      )), error = function(e) NULL)
      if (!is.null(fit)) {
        pr[!tr] <- predict(fit,
          type = "response",
...
if (nrow(rev_df) > 30 && length(unique(rev_df$reversed)) == 2) {
  ...
  X7 <- cbind(
    abs_lfc_aging = rev_df$abs_lfc_aging,
...
summ <- map_dfr(results, function(r) {
  tibble(
    classifier = r$name, n = r$n, n_pos = r$n_pos, n_neg = r$n_neg,
