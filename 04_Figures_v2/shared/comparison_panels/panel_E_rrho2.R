...
if (length(na_rows) && length(na_cols)) {
  row_before <- seq_len(min(na_rows) - 1)
  row_after <- (max(na_rows) + 1):nr
  col_before <- seq_len(min(na_cols) - 1)
  col_after <- (max(na_cols) + 1):nc
} else {
  mid <- floor(nr / 2)
  row_before <- seq_len(mid)
  row_after <- (mid + 1):nr
  col_before <- seq_len(mid)
  col_after <- (mid + 1):nc
}
...
hmat_df <- expand.grid(row = seq_len(nr), col = seq_len(nc)) |>
  ...
  if (nrow(res) > 0) {
    res |>
      mutate(
        ...
        geneID = vapply(overlapGenes, paste, character(1), collapse = "/")
      ) |>
      arrange(padj, size)
  } else {
    tibble()
...
og <- cfg$ora_grouped
ora_by_quad <- list(
  ora_UU = ora_UU, ora_DD = ora_DD,
  ora_UD = ora_UD, ora_DU = ora_DU
)
ora_group_1 <- bind_rows(ora_by_quad[og$file_1_quads])
ora_group_2 <- bind_rows(ora_by_quad[og$file_2_quads])
...
if (!is.null(og$note_if_empty_2) && nrow(ora_group_2) == 0) {
  write_csv(
    tibble(note = og$note_if_empty_2),
