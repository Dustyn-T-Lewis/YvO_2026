...
message(sprintf(
  "  %d total pathways (Hallmark: %d, GO Slim: %d) | %d significant",
  nrow(fgsea_wide),
...
nes_ci_all <- fisher_z_ci(nes_cor_all$estimate, nrow(fgsea_wide), spearman = TRUE)
...
pD <- ggplot(mapping = aes(x = .data[[nes_x]], y = .data[[nes_y]])) +
  ...
  scale_size_continuous(
    range = c(1.5, 5), name = "Set size",
    breaks = c(20, 50, 100, 200)
  ) +
  geom_label_repel(
    data = label_pw, aes(label = pathway_label),
...
message(sprintf("%s Panel D done", cfg$fig_id))
