#!/usr/bin/env Rscript
# Shared Panel A ORA engine — Panel A in F04 (concordance) and F05 (reversal).
# Scatter of logFC_x vs logFC_Training_Old with threshold-free quadrant ORA
# bars flanking each corner. Driven by a global `cfg` list (see F04/F05
# a_script/_panel_A_ORA.R wrappers). Red corners = same-direction, blue = off.
# Exports: composite, scatter_df, n_total, n_sig, n_enrich, r_spear, r_pear.

source("04_Figures_v2/shared/print_scale_apply_380mm.R")
...
# Resolved here (not in the wrapper) so the print-scaled style constants apply.
bar_axis_title_size <- if (identical(cfg$bar_axis_title_mode, "raw5")) {
  5 * PRINT_SCALE
} else {
  FIG_AXIS_TEXT
}
...
int_col <- cfg$pi_int_col
scatter_df <- dep_df |>
  transmute(gene,
    logFC_x = .data[[cfg$x_logfc_col]],
    logFC_TO = logFC_Training_Old,
    pi_x = .data[[cfg$pi_x_col]],
    pi_TO = pi_score_Training_Old,
    pi_int = if (!is.null(int_col)) .data[[int_col]] else NA_real_
  ) |>
  filter(!is.na(logFC_x), !is.na(logFC_TO)) |>
  left_join(imputation_df, by = "gene") |>
  mutate(
    imputed = replace_na(imputed, FALSE),
    sig_class = cfg$classify_fn(pi_x, pi_TO, pi_int),
    is_sig = sig_class != "NS",
    quadrant = case_when(
      logFC_x > 0 & logFC_TO > 0 ~ cfg$quad_names$TR,
      logFC_x < 0 & logFC_TO < 0 ~ cfg$quad_names$BL,
      logFC_x > 0 & logFC_TO < 0 ~ cfg$quad_names$BR,
      TRUE ~ cfg$quad_names$TL
    )
  )
...
ora_by_corner <- lapply(cfg$quad_names[c("TR", "TL", "BL", "BR")], function(nm) {
  run_set_ora(scatter_df$gene[scatter_df$quadrant == nm], nm)
})
names(ora_by_corner) <- c("TR", "TL", "BL", "BR")

all_quad_ora <- bind_rows(ora_by_corner[cfg$ora_bind_order])
...
q_df <- scatter_df |>
  mutate(q = case_when(
    logFC_x > 0 & logFC_TO > 0 ~ "TR",
    logFC_x < 0 & logFC_TO < 0 ~ "BL",
    logFC_x > 0 & logFC_TO < 0 ~ "BR",
    TRUE ~ "TL"
  ))
q_counts <- q_df |>
  count(q) |>
...
q_sig <- q_df |>
  ...
  count(q) |>
  deframe()
for (qq in c("TR", "TL", "BL", "BR")) if (is.na(q_sig[qq])) q_sig[qq] <- 0

label_df <- sig_df |>
  group_by(sig_class) |>
  arrange(desc(abs(logFC_x) + abs(logFC_TO))) |>
  slice_head(n = 5) |>
  ungroup() |>
  mutate(
    label_fill = cfg$sig_label_fill[as.character(sig_class)],
    label_text_col = cfg$sig_label_text[as.character(sig_class)]
  )
...
p_scatter <- ggplot(mapping = aes(x = logFC_x, y = logFC_TO)) +
  annotate("rect",
  ...
  geom_label_repel(
    ...
    box.padding = 0.3, point.padding = cfg$repel_point_padding,
  ...
  annotate("label",
    ...
    label = sprintf("%s\n%s/%s", cfg$quad_disp$TR, q_sig["TR"], q_counts["TR"]),
  ...
  annotate("label",
    ...
    label = sprintf("%s/%s\n%s", q_sig["BL"], q_counts["BL"], cfg$quad_disp$BL),
  ...
  annotate("label",
    ...
    label = sprintf("%s\n%s/%s", cfg$quad_disp$TL, q_sig["TL"], q_counts["TL"]),
  ...
  annotate("label",
    ...
    label = sprintf("%s/%s\n%s", q_sig["BR"], q_counts["BR"], cfg$quad_disp$BR),
...
key_lvls <- cfg$key_lvls
key_display <- cfg$key_display
key_df <- tibble(
  category = factor(key_lvls, levels = key_lvls),
  display = key_display,
  fill_col = unname(cfg$sig_colors[key_lvls]),
  x = cfg$key_x,
  y = 0
)
p_key <- ggplot(key_df, aes(x = x, y = y)) +
  geom_point(aes(fill = category),
    shape = 21, size = 2.5 * PRINT_SCALE,
...
# Half-bar builder
force_outside <- cfg$force_outside %||% character(0)
force_outside_right <- cfg$force_outside_right %||% character(0)
...
make_half_bars <- function(df, fill_color, side, ylim,
  ...
  bars <- bars |>
    mutate(
      label_inside = neg_log10_padj >= x_max * cfg$bar_label_inside_frac &
  ...
  p <- ggplot(bars, aes(y = y)) +
    ...
    theme(
      ...
      axis.title.x = if (!is_upper) {
        element_text(
          size = bar_axis_title_size, face = "bold",
          margin = margin(t = 0, unit = "mm")
        )
      } else {
        element_blank()
...
p_ul <- make_half_bars(ora_by_corner$TL, scales::alpha(COMP_BLUE, 0.30), "left",
  c(0, 2.8),
  display_labels = DISPLAY_LABELS
)
p_ll <- make_half_bars(ora_by_corner$BL, scales::alpha(COMP_RED, 0.30), "left",
  c(-2.8, 0),
  display_labels = DISPLAY_LABELS
)
p_ur <- make_half_bars(ora_by_corner$TR, scales::alpha(COMP_RED, 0.30), "right",
  c(0, 2.8),
  display_labels = DISPLAY_LABELS
)
p_lr <- make_half_bars(ora_by_corner$BR, scales::alpha(COMP_BLUE, 0.30), "right",
  c(-2.8, 0),
  display_labels = DISPLAY_LABELS
)
...
r_spear <- cor(scatter_df$logFC_x, scatter_df$logFC_TO,
  use = "complete.obs",
  method = "spearman"
)
sig_sub <- scatter_df |> filter(is_sig)
r_pear <- cor(sig_sub$logFC_x, sig_sub$logFC_TO, use = "complete.obs")
...
composite <- p_ul + p_scatter + p_ur + p_ll + p_lr + p_key +
  ...
  plot_annotation(
    title = cfg$title,
    subtitle = sprintf("Threshold-free ORA (hypergeometric) | N = %d | %d DEPs (\u03a0 < 0.05) | %d enriched (FDR < 0.05) | \u03c1(all) = %.2f, r(sig) = %.2f", n_total, n_sig, n_enrich, r_spear, r_pear),
    theme = theme(
...
message(sprintf("\n%s Panel A ORA composite done", cfg$fig_id))
