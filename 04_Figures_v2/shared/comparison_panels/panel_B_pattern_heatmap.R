...
divider_ys <- quad_ends[seq_len(length(QUAD_ORDER) - 1)]
...
pB <- ggplot() +
  ...
  annotate("segment",
    x = X_BAR_L - 0.05, xend = X_BAR_MAX + (cfg$bg_extend_right %||% 2.0),
  ...
  annotate("text",
    x = X_BAR_L + cfg$protein_count_x_mult * BAR_SCALE,
