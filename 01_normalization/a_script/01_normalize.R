#!/usr/bin/env Rscript
# Stage 01: Normalize
# HPA filter → blood removal → dedup → missingness → outlier detection →
# missingness re-applied → cycloess
#
# Outputs:
...
filter_log <- tibble(
  step = "Raw input", n_before = NA_integer_,
...
stopifnot(
  "align_data_and_metadata reordered samples" =
    identical(colnames(aligned$data), colnames(int_mat)),
  "annotation rows are not aligned to data rows" =
    identical(rownames(annot_df), rownames(int_mat))
)
int_mat <- aligned$data
meta_df <- aligned$metadata
...
miss_info <- dal$metadata |>
  ...
  mutate(
    delta_missing = if (n() == 2) abs(diff(pct_missing)) else NA_real_,
    .by = prefix
  )
...
if (n_outliers > 0) {
  ...
  # Re-apply on the samples that remain. The first pass ran on all 64, and
  # outlier detection needs a filtered matrix to work on, so the order cannot
...
message(sprintf(
  "Normalized: %d proteins x %d samples",
...
pheno <- pheno[!is.na(part_id) & part_id != "" &
                 !part_id %in% REPORT_FURNITURE &
                 !startsWith(part_id, "T-test"), , drop = FALSE]
stopifnot(nrow(pheno) == 32L)
...
filter_log <- mutate(filter_log, pct_of_raw = round(n_after / n_raw * 100, 1))
...
add_overview(
  wb,
  title = "S8 Table \u2014 normalization",
  description = paste(
    "Every sample and protein that entered the analysis, and the filtering and",
    "outlier decisions that produced them."
  ),
  entries = c(
    sample_metadata     = "One row per biopsy: sample identifier, participant, age group and timepoint.",
    phenotype           = "One row per participant: age, supplement arm, body composition and strength measures.",
    normalized_matrix   = "Cyclic-loess normalized log2 intensity, one row per protein and one column per sample.",
    filter_log          = "How many proteins each filtering step removed, and how many remained.",
    outlier_diagnostics = "Per-sample outlier scores from four methods, and which samples the consensus dropped.",
    filtered_proteins   = "The proteins removed by filtering, with the reason each one failed.",
    protein_missingness = "Missing-value count and percentage for every protein.",
    sample_missingness  = "Missing-value count and percentage for every sample."
  )
)
