#!/usr/bin/env Rscript
# SUPP Panel D: shared-baseline coupling diagnostic.
#
...
PW <- 89
PH <- 70
ggsave(file.path(RPT_PNG, "SUPP_coupling_null.png"), pS_coupling,
  width = PW, height = PH, units = "mm", dpi = 300
)
ggsave(file.path(RPT_PDF, "SUPP_coupling_null.pdf"), pS_coupling,
  width = PW, height = PH, units = "mm", device = pdf_device
)

pS_coupling <- strip_for_composite(pS_coupling)
