#!/usr/bin/env Rscript
# SUPP: corner enrichment against list depth (CAT curve)
...
PW_CAT <- 89
PH_CAT <- 70
ggsave(file.path(RPT_PNG, "SUPP_cat_depth.png"), pS_cat,
  width = PW_CAT, height = PH_CAT, units = "mm", dpi = 300
)
ggsave(file.path(RPT_PDF, "SUPP_cat_depth.pdf"), pS_cat,
