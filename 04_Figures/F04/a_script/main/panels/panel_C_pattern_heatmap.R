...
# Outputs: MAIN_panel_C_pattern_heatmap.{pdf,png}
...
sig_df <- sig_df %>%
  ...
  arrange(quadrant, pathway, desc(logFC_Training_Young))
...
dom_quad <- sig_df %>%
  ...
  summarise(n = n(),
            lfc_sum = sum(abs(logFC_Training_Young) + abs(logFC_Training_Old)),
            .groups = "drop") %>%
  group_by(pathway) %>%
  mutate(is_max = n == max(n), n_tied = sum(is_max)) %>%
  # For ties: pick quadrant with largest total |logFC|
...
# =============================================================================
# 4. X-COORDINATE LAYOUT — x-axis to ~20
# =============================================================================
...
# Striped background — extends to X_BAR_MAX
...
# Pathway labels at end of bars, wrapped
pw_labels <- pw_counts %>%
...
# =============================================================================
# 6. SANKEY — targets map to fractional y-bands
# =============================================================================
flow_df <- sig_df %>%
  filter(pathway %in% pw_counts$pathway) %>%
...
# 7. HEATMAP TILE DATA
# =============================================================================
fc_max <- max(abs(c(sig_df$logFC_Training_Young, sig_df$logFC_Training_Old)), na.rm = TRUE)
...
heat_tiles <- bind_rows(
  sig_df %>% transmute(x = X_COL1, y, w = TILE_W, h = ROW_H,
                        fill = lfc_to_color(logFC_Training_Young, fc_max)),
...
# =============================================================================
n_g <- 50
# Compact gradient bar (RRHO-style: short, centered below heatmap)
...
KEY_X_SIG   <- bar_mid - 1.43
KEY_X_QUAD  <- bar_mid + 0.6

sig_cats_f04 <- c("Tr.(Y)", "Tr.(O)", "Both", "Inter.")
sig_key_df <- tibble(
  x     = KEY_X_SIG,
  y     = KEY_Y_BASE + KEY_DY * seq_along(sig_cats_f04),
  label = c("Sig Young only", "Sig Old only", "Sig Both", "Interaction"),
  fill  = SIG_COLORS[sig_cats_f04]
)
