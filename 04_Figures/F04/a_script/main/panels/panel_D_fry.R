# F04 Panel D — fry Rotation Test: Training Concordance Barcode
# Tests whether Training-Young-significant protein sets respond concordantly
# in Old subjects, using limma's fry gene-set rotation test.
#
...
#
# No circularity: Training_Young (Young_Post - Young_Pre) and Training_Old
# (Old_Post - Old_Pre) share no contrast terms.
#
...
# -- Step 4: Define Training_Young gene sets -----------------------------------

imp_ids <- rownames(mat_imp)

sig_pi <- dep_df %>%
  filter(pi_score_Training_Young < 0.05, uniprot_id %in% imp_ids)

sets_pi <- list(
  up      = match(sig_pi$uniprot_id[sig_pi$logFC_Training_Young > 0], imp_ids),
  down    = match(sig_pi$uniprot_id[sig_pi$logFC_Training_Young < 0], imp_ids),
  up_ids  = sig_pi$uniprot_id[sig_pi$logFC_Training_Young > 0],
  down_ids = sig_pi$uniprot_id[sig_pi$logFC_Training_Young < 0]
)
...
fry_up <- run_fry_set(sets_pi$up, "ty_up") %>%
  mutate(expected = "Up", consistent = direction == expected)
fry_dn <- run_fry_set(sets_pi$down, "ty_down") %>%
...
driving_up <- dep_df %>%
  filter(uniprot_id %in% sets_pi$up_ids, uniprot_id %in% imp_ids,
         t_Training_Old > 0) %>%
  transmute(gene, uniprot_id, set = "ty_up",
            t_training_young = t_Training_Young, t_training_old = t_Training_Old,
            logFC_Training_Young, logFC_Training_Old, pi_score_Training_Young)

driving_dn <- dep_df %>%
  filter(uniprot_id %in% sets_pi$down_ids, uniprot_id %in% imp_ids,
         t_Training_Old < 0) %>%
  transmute(gene, uniprot_id, set = "ty_down",
            t_training_young = t_Training_Young, t_training_old = t_Training_Old,
            logFC_Training_Young, logFC_Training_Old, pi_score_Training_Young)
...
# Contrast colors for the two sets
UP_COLOR   <- unname(CONTRAST_COLORS["Training_Young"])  # red-ish
DOWN_COLOR <- unname(CONTRAST_COLORS["Training_Old"])    # blue-ish
...
# Build title strings
up_title <- sprintf("Tr.(Y)-Up DEPs (\u03a0 < 0.05, n = %d) \u2192 Tr.(O) ranked t",
                     length(sets_pi$up))
dn_title <- sprintf("Tr.(Y)-Down DEPs (\u03a0 < 0.05, n = %d) \u2192 Tr.(O) ranked t%s",
                     length(sets_pi$down),
                     if (fry_dn$PValue > 0.05) "  (n.s.)" else "")

# F04 concordance: both curves red
...
p_t <- ggplot(t_rank, aes(x = rank, y = t_Training_Old)) +
  geom_area(fill = scales::alpha(CONTRAST_COLORS["Training_Old"], 0.20),
            color = CONTRAST_COLORS["Training_Old"], linewidth = 0.3) +
...
shorten_ora_label <- function(x, max_chars = 28, explicit_map = NULL) {
  ...
  # Wrap long labels with newline instead of truncating
...
p_ora_flank_up <- make_flanking_ora(ora_leading_up, "Concordant (Up)",
                                     unname(DIR_COLORS["Up"]),
                                     label_map = F04_LABEL_MAP)
p_ora_flank_dn <- make_flanking_ora(ora_leading_dn, "Concordant (Down)",
...
pD_fry <- p1$es + p1$bc + p2$es + p2$bc + p_t +
  ...
  plot_annotation(
    ...
    subtitle = sprintf("Rotation-based set test (exact GSEA analogue, accounts for inter-gene correlation) | dupCor = %.3f | n = %d proteins",
                        cor_imp, n_all),
...
if (!is.null(p_ora_up) || !is.null(p_ora_dn)) {
  ...
  message("F04 Panel D ORA (supp) done")
}

# --- Export for composite ---
pD_title    <- "fry Gene-Set Rotation Test: Training Concordance"
pD_subtitle <- sprintf("Rotation-based set test (exact GSEA analogue, accounts for inter-gene correlation) | dupCor = %.3f | n = %d proteins",
                        cor_imp, n_all)
pD_legend   <- NULL
