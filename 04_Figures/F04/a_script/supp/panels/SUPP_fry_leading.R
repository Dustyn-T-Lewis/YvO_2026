...
# ── Data ─────────────────────────────────────────────────────────────────────
# Try reading driving proteins from the supplementary xlsx first
...
if (is.null(driving_df)) {
  ...
  dep_df <- as.data.frame(readr::read_csv("03_DEP/c_data/03_combined_results.csv", show_col_types = FALSE))
