#!/usr/bin/env Rscript
# S5b Figure: protein-level training response across age
...
#
# Returns the drawing as a grob with its size attached as attr(, "width_mm")
# and attr(, "height_mm").
...
source("04_Figures/F04/a_script/supp/panels/_young_dep_ora_flow.R", local = TRUE)
...
attr(p, "width_mm") <- fig_w_mm
attr(p, "height_mm") <- fig_h_mm
invisible(p)
