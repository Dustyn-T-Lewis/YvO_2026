...
for (quad in QUADRANTS) {
  ...
  fname <- file.path(RPT_PNG, sprintf("SUPP_panel_G_chord_%s.png",
                                    tolower(gsub("[/ +]", "_", quad))))
...
if (length(valid_files) >= 2) {
  ...
  ggsave(file.path(RPT_PNG, "SUPP_panel_G_chord.png"), composite,
         width = COMP_W, height = COMP_H, units = "mm", dpi = 300)
  ggsave(file.path(RPT_PDF, "SUPP_panel_G_chord.pdf"), composite,
