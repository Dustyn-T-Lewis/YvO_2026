...
PANELS_DIR <- "04_Figures/F04/b_reports/supp/png/panels"
...
render_legend_strip <- function(path, width_mm = 460, height_mm = 55) {
  ...
  text(0.68, 0.90, "Pathway NES (outer arc, panels B\u2013D)",
...
# Also write legend to PDF panels dir
shared_legend_pdf_dir <- "04_Figures/F04/b_reports/supp/pdf/panels"
dir.create(shared_legend_pdf_dir, recursive = TRUE, showWarnings = FALSE)
render_legend_strip(file.path(shared_legend_pdf_dir, "SUPP_shared_legend_chord.png"))
...
ggsave(file.path(RPT_PNG, "SUPP_F04_composite.png"), composite,
       width = COMP_W, height = COMP_H, units = "mm", dpi = 300)

message(sprintf("F04 SUPP composite saved -> %s (%d panels)",
                RPT_PNG, length(panels)))
