#!/usr/bin/env Rscript
# S5 Table: draws the six S5 Table plots no figure shows, then folds their CSVs
# and the ones F04.R, S5a.R and S5b.R leave in c_data into one workbook, and
# deletes them. Run after those three.
...
# These plot sheets of this workbook and nothing else, so they run here.
for (f in c(
...
# build_workbook() skips a missing CSV without a word, which would ship S5
# Table short of sheets and then delete the rest. Stop instead.
missing_csv <- Filter(Negate(file.exists), unlist(lapply(f04_specs, `[[`, "path")))
if (length(missing_csv)) {
  stop(
    "missing intermediates; run F04.R, S5a.R and S5b.R first:\n  ",
    paste(missing_csv, collapse = "\n  ")
  )
}
build_workbook(
  file.path(DAT, "F04_data.xlsx"),
