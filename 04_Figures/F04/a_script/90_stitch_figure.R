...
# Outputs: F04_composite_MAIN.pdf/.png
#
...
source("04_Figures/F04/a_script/panels/panel_A_ORA.R")        # -> composite (patchwork)
# Snapshot Panel A stats before Panel C overwrites `n_total` and `sig_df`
n_total_A    <- nrow(scatter_df)
n_sig_A      <- n_sig
n_enrich_A   <- n_enrich
r_pear_A     <- r_pear
source("04_Figures/F04/a_script/panels/panel_B_nes_scatter.R")  # -> pB (ggplot)
...
source("04_Figures/F04/a_script/panels/panel_D_fry.R")        # -> pD_fry, p1, p2, p_t
# Snapshot Panel D stats
cor_imp_D    <- cor_imp
n_all_D      <- n_all
source("04_Figures/F04/a_script/panels/panel_E_rrho2.R")      # -> pE_heat (ggplot)
# Snapshot Panel E stats
n_shared_E   <- n_shared
max_UU_E     <- max_UU
n_UU_E       <- n_UU
source("04_Figures/F04/a_script/panels/panel_C_pattern_heatmap.R")  # -> p (ggplot, theme_void)
...
BOX_DIR     <- "/Users/dtl0018/Library/CloudStorage/Box-Box/YvO_proteomics_manuscript_2026-04-16/02_Figures/F04_training_concordance"
...
ggsave(file.path(RPT, "MAIN_F04_composite.pdf"), composite_final,
       width = COMP_W, height = COMP_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "MAIN_F04_composite.png"), composite_final,
       width = COMP_W, height = COMP_H, units = "mm", dpi = 300)
file.copy(file.path(RPT, "MAIN_F04_composite.pdf"), file.path(WRITING_DIR, "01_main_composite_f04.pdf"), overwrite = TRUE)
file.copy(file.path(RPT, "MAIN_F04_composite.png"), file.path(WRITING_DIR, "01_main_composite_f04.png"), overwrite = TRUE)
file.copy(file.path(RPT, "MAIN_F04_composite.png"), file.path(BOX_DIR, "F04_main.png"), overwrite = TRUE)
file.copy(file.path(RPT, "MAIN_F04_composite.pdf"), file.path(BOX_DIR, "pdf", "F04_main.pdf"), overwrite = TRUE)
...
file.copy("04_Figures/F04/c_data/F04_supplementary.xlsx", file.path(WRITING_DIR, "04_supp_data_f04.xlsx"), overwrite = TRUE)
