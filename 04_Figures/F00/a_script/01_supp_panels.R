...
pA <- ggplot(fcasc, aes(step, n_after)) +
  ...
  labs(x = NULL, y = "Proteins retained", tag = "A",
...
pB <- ggplot(miss_hist_df, aes(pct_miss)) +
  ...
  labs(x = "% missing", y = "Proteins", tag = "B",
...
pE <- ggplot(eta_df, aes(eta2)) +
  ...
  geom_vline(xintercept = eta_90, linetype = "dashed",
...
pF <- ggplot(od, aes(mahal_dist, n_flags, color = status, shape = Timepoint)) +
  ...
  scale_color_manual(values = c(Clean = "grey55", Flagged = "#E6A100",
                                Outlier = "#B2182B"), name = NULL) +
  scale_shape_manual(values = SHAPE_TP, guide = "none") +
  scale_y_continuous(breaks = 0:4) +
  labs(x = "Mahalanobis distance", y = "QC flags", tag = "F",
       title = "Outlier detection consensus",
       subtitle = sprintf("%d/%d removed: %s",
...
pG <- ggplot(miss_bar, aes(reorder(Col_ID, -n), n, fill = Group_Time)) +
  ...
  labs(x = NULL, y = "Missing proteins", tag = "G",
...
pH <- ggplot(mc, aes(mean_intensity, pct_miss, color = classification)) +
  ...
  labs(x = "Mean log2 intensity", y = "% missing", tag = "H",
       title = "MAR vs. MNAR classification",
       subtitle = sprintf("%s | %s proteins", int_imp$classification_method,
                          comma(n_total))) +
  FIG_THEME + theme(legend.position = "top", legend.key.size = unit(2.5, "mm"),
                    legend.text = element_text(size = 5.5))
...
pJ <- ggplot(bench_plot, aes(composite, method, fill = bar_col)) +
  ...
  scale_fill_manual(values = c(Selected = "#E41A1C", `Top 5` = "#377EB8",
  ...
  labs(x = "Composite score", y = NULL, tag = "J",
       ...
       subtitle = sprintf("missForest selected (rank #%d, composite = %.3f)",
...
pK <- ggplot(dens_df, aes(value, fill = type, color = type)) +
  ...
  labs(x = "log2 intensity", y = "Density", tag = "K",
...
pL <- ggplot(audit, aes(shift)) +
  ...
  labs(x = "Shift (log2)", y = "MNAR proteins", tag = "L",
...
pN <- ggplot(dep_counts, aes(threshold, contrast, fill = n)) +
  ...
  labs(x = NULL, y = NULL, tag = "N",
       ...
       subtitle = sprintf("limma + duplicateCorrelation | %s proteins",
...
sl <- function(p) {
  p + labs(subtitle = NULL) +
    theme(plot.title = element_text(face = "bold", size = COMP_TITLE_SZ,
                                    margin = margin(b = 1)),
...
page1 <- (sl(pA) | sl(pB)) /
  ...
  plot_annotation(
    ...
    subtitle = sprintf("DIA-NN \u2192 HPA filter \u2192 cyclic loess \u2192 outlier removal | %s \u2192 %s proteins \u00d7 %d samples",
    ...
    theme = theme(plot.title    = element_text(face = "bold", size = txt$title),
                  plot.subtitle = element_text(face = "italic", size = txt$subtitle - 1,
                                               color = "grey30"))) &
...
page2 <- (sl(pH) | sl(pI)) /
  ...
  plot_annotation(
    ...
    subtitle = sprintf("missForest (OOB = %.3f) | %d MAR + %d MNAR proteins | limma + duplicateCorrelation",
