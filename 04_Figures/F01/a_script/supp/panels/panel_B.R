...
pheno_long <- meta %>%
  select(subject_key, Group, Timepoint, Type_II_fCSA) %>%
  filter(!is.na(Type_II_fCSA))

# Drop subjects missing either timepoint before paired t-test
pheno_wide <- meta %>%
  select(subject_key, Group, Timepoint, Type_II_fCSA) %>%
  pivot_wider(names_from = Timepoint, values_from = Type_II_fCSA, names_sep = "_") %>%
...
stats_anova <- rstatix::anova_test(data = pheno_long_complete, dv = Type_II_fCSA,
                                    wid = subject_key,
...
plot_long <- meta %>%
  filter(subject_key %in% pheno_wide$subject_key, !is.na(Type_II_fCSA))
...
pSB_right <- ggplot(pheno_wide, aes(x = Group, y = delta_fCSA, fill = Group)) +
  ...
  labs(y = expression(bold(Delta ~ "Type II fCSA (" * mu * m^2 * ")")),
       x = NULL) +
...
pSB_title    <- "Type II Fiber CSA"
pSB_subtitle <- full_sub
pSB_legend   <- NULL
pSB_left     <- strip_for_composite(pSB_left)
pSB_right    <- strip_for_composite(pSB_right)
