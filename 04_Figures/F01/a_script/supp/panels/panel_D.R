# F01 Supplementary Panel D: Deadlift 1RM
...
write.csv(audit_D, file.path(DAT, "panel_D_deadlift_1rm.csv"), row.names = FALSE)
...
pD_left <- ggplot(plot_long, aes(x = Group_Time, y = deadlift_1rm_kg, fill = Group_Time)) +
  ...
  labs(title = "Deadlift 1RM", subtitle = full_sub,
       y = "Deadlift 1RM (kg)", x = NULL, tag = "D") +
  FIG_THEME +
...
pD_right <- ggplot(pheno_wide, aes(x = Group, y = delta_DL, fill = Group)) +
...
ggsave(file.path(RPT, "SUPP_panel_D_deadlift_1rm.png"), pD,
       width = PW, height = PH, units = "mm", dpi = 300)
cat("F01 Supp Panel D done\n")
