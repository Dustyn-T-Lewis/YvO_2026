...
DAT_F06 <- "04_Figures/F06/c_data"
OUT     <- "04_Figures/F07/c_data/roc_pilot"
dir.create(OUT, recursive = TRUE, showWarnings = FALSE)

# ---- Load data ---------------------------------------------------------------
MEs     <- readRDS(file.path(DAT_F06, "MEs.rds"))
me_pre  <- readRDS(file.path(DAT_F06, "me_pre.rds"))
me_post <- readRDS(file.path(DAT_F06, "me_post.rds"))
subj_age<- read_csv(file.path(DAT_F06, "subj_age.csv"), show_col_types = FALSE)
shared  <- readRDS(file.path(DAT_F06, "shared_objects.rds"))
common_subj <- shared$common_subj
pheno   <- read_csv(file.path(DAT_F06, "pheno_wide.csv"), show_col_types = FALSE)

dep   <- read_csv("03_DEP/c_data/03_combined_results.csv", show_col_types = FALSE)
mods  <- read_csv(file.path(DAT_F06, "wgcna/wgcna_module_assignments.csv"),
...
eval_clf <- function(name, labels, X, k_range, n_perm = 200) {
  ...
  list(name=name, auc=auc_v, ci_lo=ci[1], ci_hi=ci[3],
       perm_p=mean(nulls >= auc_v), probs=res$probs, labels=labels,
...
X_pheno <- pheno %>% filter(subject_key %in% common_subj) %>%
  select(subject_key, VL_Pre, LBM_Pre, BMI_Pre) %>%
...
prot_feat <- dep %>%
  ...
  left_join(mods %>% select(uniprot_id, module_color), by="uniprot_id")
...
results$prot_dep <- list(
  ...
  perm_p=mean(null6 >= as.numeric(auc(roc6))),
...
if (nrow(rev_df) > 30 && length(unique(rev_df$reversed)) == 2) {
  ...
  results$rev_vs_exa <- list(
    ...
    perm_p=mean(null7 >= as.numeric(auc(roc7))),
