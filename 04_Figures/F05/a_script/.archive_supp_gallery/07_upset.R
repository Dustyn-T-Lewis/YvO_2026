...
SET_NAMES <- c("Aging", "Training (Old)")
SET_COLORS <- c(
...
bin_mat <- cbind(
  Aging            = as.integer(rev$sig_Aging),
  `Training (Old)` = as.integer(rev$sig_TO)
)
rownames(bin_mat) <- rev$pathway
...
p_bars <- ggplot(bar_df, aes(x, count)) +
  ...
  labs(y = "Intersection\nsize",
       ...
       subtitle = "Aging Reversal: sig pathways across contrasts") +
  FIG_THEME +
...
ggsave(file.path(RPT, "a_upset.pdf"), p_combined,
       width = PW, height = 120, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "a_upset.png"), p_combined,
       width = PW, height = 120, units = "mm", dpi = 300)
