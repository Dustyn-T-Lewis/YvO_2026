...
# F05 Panel A ORA: Scatter + Flanking ORA Bars Composite
...
# F05-specific pathway label shortenings (used in the half-bar panels below)
DISPLAY_LABELS_F05 <- c(
...
if (file.exists(imputation_path)) {
  ...
  imputation_df <- tibble(gene = character(), imputed = logical())
}

scatter_df <- dep_df |>
  transmute(gene,
            logFC_Aging = logFC_Aging, logFC_TO = logFC_Training_Old,
  ...
  mutate(
    ...
    sig_class = classify_proteins_f3(pi_Aging, pi_TO),
    is_sig    = sig_class != "NS",
    quadrant  = case_when(
      logFC_Aging > 0 & logFC_TO < 0 ~ "Reversed (Aging Up / Training Down)",
      logFC_Aging < 0 & logFC_TO > 0 ~ "Reversed (Aging Down / Training Up)",
...
ora_tl <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Reversed (Aging Down / Training Up)"],
                       "Reversed (Aging Down / Training Up)")
ora_tr <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Exacerbated Up"],
                       "Exacerbated Up")
ora_bl <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Exacerbated Down"],
                       "Exacerbated Down")
ora_br <- run_set_ora(scatter_df$gene[scatter_df$quadrant == "Reversed (Aging Up / Training Down)"],
                       "Reversed (Aging Up / Training Down)")
...
q_df <- scatter_df |>
  mutate(q = case_when(
    logFC_Aging > 0 & logFC_TO < 0 ~ "BR",
    logFC_Aging < 0 & logFC_TO > 0 ~ "TL",
    logFC_Aging > 0 & logFC_TO > 0 ~ "TR",
    TRUE ~ "BL"))
...
label_df <- sig_df |>
  group_by(sig_class) |>
  arrange(desc(abs(logFC_Aging) + abs(logFC_TO))) |>
  slice_head(n = 5) |>
  ungroup() |>
...
p_scatter <- ggplot(mapping = aes(x = logFC_Aging, y = logFC_TO)) +
  ...
  annotate("label", x = xlim_range[2], y = ylim_range[1],
           label = sprintf("%s/%s\nReversed (Age\u2191 Tr\u2193)", q_sig["BR"], q_counts["BR"]),
...
r_spear  <- cor(scatter_df$logFC_Aging, scatter_df$logFC_TO, use = "complete.obs",
                method = "spearman")
sig_sub  <- scatter_df |> filter(is_sig)
r_pear   <- cor(sig_sub$logFC_Aging, sig_sub$logFC_TO, use = "complete.obs")
