...
# The cache is only valid for the network it was computed on. Without this,
# a cache from an earlier matrix is loaded silently and the Z-summaries
# describe modules that no longer exist.
net_inputs <- file.path(DAT, c("datExpr.rds", "module_colors.rds"))
fresh <- file.exists(pres_cache) &&
  file.mtime(pres_cache) >= max(file.mtime(net_inputs))
if (fresh) {
  ...
  mp <- modulePreservation(
    ...
    nPermutations     = 200,
    randomSeed        = 42,
    # Default TRUE dumps permutedStats-actualModules.RData into the repo root.
