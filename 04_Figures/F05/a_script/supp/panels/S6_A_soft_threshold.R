#!/usr/bin/env Rscript
# S6 Figure A: scale-free topology fit and mean connectivity by soft power.
...
p1 <- ggplot(fit_df, aes(power, r2)) +
  ...
  # 0.965, not 0.88: the curve plateaus at 0.86 to 0.90 across powers 13-17,
  # so the label sat on top of the points it was meant to explain.
  annotate("text", x = 19, y = 0.965,
...
ggsave(file.path(RPT_PNG, "S6_A_soft_threshold.png"), composite,
       width = PA_W, height = PA_H, units = "mm", dpi = 300)
ggsave(file.path(RPT_PDF, "S6_A_soft_threshold.pdf"), composite,
