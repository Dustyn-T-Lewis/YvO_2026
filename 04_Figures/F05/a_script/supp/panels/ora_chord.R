# F05 supplementary: Unified ORA-DEP Chord Diagram — Aging
# Single chord connecting ORA-enriched pathways to "Aging" DEPs
# (Aging + Training_Old, Pi < 0.05 in either).
#
# STATUS: supplementary artifact, NOT wired into the main F05 composite.
...
FIGURE_LABEL       <- "Aging"
RELEVANT_CONTRASTS <- c("Aging", "Training_Old")
...
write_csv(
  ...
  file.path(DAT, "panel_E", "chord_aging_combined.csv")
)

write_csv(
  pw_direction %>% select(pathway_label, pathway_padj, pathway_db,
                            n_overlap, mean_logFC, direction),
  file.path(DAT, "panel_E", "chord_aging_pathways.csv")
)
...
outfile_pdf <- file.path(RPT_PDF, "SUPP_ora_chord.pdf")
render_chord(outfile_pdf, "pdf", mode = "full")
message(sprintf("Rendered: %s", outfile_pdf))
...
message(sprintf("\nDone: F05 supplementary ORA chord — %s (%d proteins, %d pathways)",
