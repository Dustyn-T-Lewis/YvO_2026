# F05 Panel F — Convergent Pathways: RRHO intersection fry ORA (Reversal)
# Identifies high-confidence reversal pathways supported by both threshold-free
...
if (!file.exists(rrho_ora_path)) {
  ...
  write_csv(tibble(note = "RRHO ORA not available"),
...
if (nrow(rrho_ora) == 0 || nrow(fry_ora) == 0 || "note" %in% names(fry_ora)) {
  ...
  write_csv(tibble(note = "No convergent pathways (one or both ORA empty)"),
            file.path(DAT, "panel_F_fry", "convergent_pathways.csv"))
  quit(save = "no", status = 0)
}
...
if (nrow(convergent) == 0) {
  ...
  write_csv(tibble(note = "No convergent pathways found"),
            file.path(DAT, "panel_F_fry", "convergent_pathways.csv"))
  quit(save = "no", status = 0)
}
...
pConv <- ggplot(conv_long, aes(x = neg_log10_padj, y = pathway_label,
  ...
  labs(
    title = expression("Convergent Pathways: RRHO" ~ intersect ~ "fry (Reversal)"),
...
ggsave(file.path(RPT, "SUPP_panel_F_convergent.png"), pConv,
       width = PE_W, height = conv_h, units = "mm", dpi = 300)

message("F05 Panel F (convergent) done")
