#!/usr/bin/env Rscript
# S6 Figure C: subcellular compartment enrichment per module.
#
# Reads module assignments and labels from c_data/F05_data.xlsx, which
# F05_data.R writes after this script has run. Each run therefore reads the
# workbook the previous run left, the same as before the split.
...
F05_SUPP <- file.path(DAT, "F05_data.xlsx")
stopifnot("F05_data.R must run first: missing F05_data.xlsx" =
  file.exists(F05_SUPP))
mods <- read_sheet_df(F05_SUPP, "WGCNA_module_assignments")

bio_labels <- tryCatch(
  read_sheet_df(F05_SUPP, "WGCNA_mod_bio_labels"),
...
validate <- function(module_label, expected_compartment) {
  ...
  if (nrow(row) == 0) {
    ...
    message(sprintf("  %s: %s -> %s (OR=%.1f, p_bh=%.4f, n=%d)",
