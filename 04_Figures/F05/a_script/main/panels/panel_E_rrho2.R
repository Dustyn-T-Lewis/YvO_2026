...
if (nrow(ora_all) > 0) {
  ...
  ggsave(file.path(RPT_SUP_PNG, "SUPP_panel_E_rrho2_ora.png"), pE_combined,
         width = 400, height = 220, units = "mm", dpi = 300)
  ggsave(file.path(RPT_SUP_PDF, "SUPP_panel_E_rrho2_ora.pdf"), pE_combined,
...
pE_title    <- "Threshold-Free Reversal (RRHO2)"
pE_subtitle <- sprintf("Stratified hypergeometric | %d shared genes | warm off-diagonal = training reverses aging | No MTC (Cahill et al. 2018)", n_shared)
pE_legend   <- NULL
