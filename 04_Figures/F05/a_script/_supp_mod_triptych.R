...
# Filenames carry the module's biology label, so a relabelled module orphans its old
# file. 90_stitch_F05.R globs this directory when it packages the supplement, which
# would ship the orphan as an extra page.
unlink(list.files(c(RPT_SUPP_PNG, RPT_SUPP_PDF), "^SUPP_triptych_", full.names = TRUE))
...
triptych_row <- function(mod, show_xlab = FALSE) {
  ...
  bar_data <- enrich |>
    ...
    mutate(
      ...
      clean_name  = stringr::str_trunc(clean_pathway_name(Description), 28, ellipsis = "\u2026"),
  ...
  if (nrow(bar_data) == 0) {
    ...
    p_bars <- ggplot(bar_data, aes(x = neg_log10_p, y = clean_name)) +
      ...
      # Beside the bar, not inside it. Inside, the term was drawn white on the
      # dark fills and ran off the end of the shorter bars, where white on the
      # panel background is invisible -- "Metabolism Of Carbo" was the whole
      # term, silently cut at the bar's edge.
      geom_text(aes(label = clean_name),
        x = max(bar_data$neg_log10_p) * 1.04, hjust = 0, size = txt_bar,
...
pathway_slug <- setNames(
  gsub("[|/ ]+", "_", gsub("\\.", "", tolower(mod_bio$bio_label))),
