...
# Outputs: F05_composite_MAIN.pdf/.png
#
...
source("04_Figures/F05/a_script/panels/panel_A_ORA.R")              # -> composite (patchwork)
# Snapshot Panel A stats before Panel C overwrites `n_total` and `sig_df`
n_total_A    <- nrow(scatter_df)
n_sig_A      <- n_sig
n_enrich_A   <- n_enrich
r_pear_A     <- r_pear
source("04_Figures/F05/a_script/panels/panel_B_nes_scatter.R")      # -> pB (ggplot)
...
source("04_Figures/F05/a_script/panels/panel_D_fry.R")              # -> pD_fry, p1, p2, p_t
# Snapshot Panel D stats
cor_imp_D    <- cor_imp
n_all_D      <- n_all
circ_r_D     <- circ_r
source("04_Figures/F05/a_script/panels/panel_E_rrho2.R")            # -> pE_heat (ggplot)
...
source("04_Figures/F05/a_script/panels/panel_C_pattern_heatmap.R")  # -> p (ggplot, theme_void)

RPT <- "04_Figures/F05/b_reports"
WRITING_DIR <- "/Users/dtl0018/Library/CloudStorage/OneDrive-AuburnUniversity/YvO_writing/Figures/F05"
BOX_DIR     <- "/Users/dtl0018/Library/CloudStorage/Box-Box/YvO_proteomics_manuscript_2026-04-16/02_Figures/F05_aging_reversal"
...
pB_clean <- pB +
  labs(title    = "Pathway Reversal",
       subtitle = sprintf("%d/%d sig. pathways | \u03c1 = %.2f [%.2f, %.2f] | %.0f%% reversed",
                          n_sig_pw_B, n_pw_B, rho_B, rho_lo_B, rho_hi_B, pw_rev_B * 100)) +
  TITLE_THEME +
...
pC_clean <- p +
  ...
  theme(plot.title    = element_text(margin = margin(t = 2, b = 1, l = 18, unit = "mm")),
        ...
        plot.margin   = margin(t = 12, r = 4, b = 8, l = -18))

# Panel D: fry barcode + flanking ORA bars (L=10mm matches individual panel)
pD_clean <- pD_fry +
  plot_annotation(
    title    = "fry: Aging Reversal",
    subtitle = sprintf("n = %d | dupCor = %.3f | circ r = %.3f",
                       n_all_D, cor_imp_D, circ_r_D),
    theme    = TITLE_THEME + theme(
...
# Panel E: RRHO2 heatmap (square) — rebuilt with RRHO2 package (matching F04)
pE_clean <- pE_heat +
  labs(title    = "RRHO2 Reversal",
       subtitle = sprintf("%d shared genes | min P < 1e-%d (%d genes)",
                          n_shared_E, round(max_rev_E), n_rev_E)) +
  TITLE_THEME +
...
ggsave(file.path(RPT, "MAIN_F05_composite.pdf"), composite_final,
       width = COMP_W, height = COMP_H, units = "mm", device = pdf_device)
ggsave(file.path(RPT, "MAIN_F05_composite.png"), composite_final,
       width = COMP_W, height = COMP_H, units = "mm", dpi = 300)
file.copy(file.path(RPT, "MAIN_F05_composite.pdf"), file.path(WRITING_DIR, "01_main_composite_f05.pdf"), overwrite = TRUE)
file.copy(file.path(RPT, "MAIN_F05_composite.png"), file.path(WRITING_DIR, "01_main_composite_f05.png"), overwrite = TRUE)
file.copy(file.path(RPT, "MAIN_F05_composite.png"), file.path(BOX_DIR, "F05_main.png"), overwrite = TRUE)
file.copy(file.path(RPT, "MAIN_F05_composite.pdf"), file.path(BOX_DIR, "pdf", "F05_main.pdf"), overwrite = TRUE)
...
file.copy("04_Figures/F05/c_data/F05_supplementary.xlsx", file.path(WRITING_DIR, "04_supp_data_f05.xlsx"), overwrite = TRUE)
file.copy("04_Figures/F05/c_data/F05_supplementary.xlsx", file.path(BOX_DIR, "F05_source_data.xlsx"), overwrite = TRUE)
