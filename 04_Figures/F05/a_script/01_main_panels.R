...
source("04_Figures/F05/a_script/_panel_A_module_heatmap.R")
source("04_Figures/F05/a_script/_panel_B_nes_scatters.R")
...
embed_pdf_fonts(file.path(RPT_PDF, "MAIN_F05_composite.pdf"))
ggsave(file.path(RPT_PNG, "MAIN_F05_composite.png"), composite_final,
  width = SAVE_W, height = SAVE_H, units = "mm",
  dpi = 300
)
message("F05 composite saved: MAIN_F05_composite.{pdf,png}")
...
# Three of the specs above read CSVs these scripts write. They used to run in
# 02_supp_panels.R, after the workbook was already built, so on a clean tree
# their sheets were missing and on a re-run they were picked up from the
# previous run's leftovers: 19 sheets the first time, 22 afterwards. They are
# independent of the supplementary composite, which is saved before them, so
# running them here costs nothing and makes the workbook reproducible.
source("04_Figures/F05/a_script/_supp_mod_triptych.R")
source("04_Figures/F05/a_script/_supp_mod_hub.R")
source("04_Figures/F05/a_script/_supp_preservation.R")

# Assembled in 90_stitch_F05.R instead, after 02_supp_panels.R has written the
# four QC CSVs the last four specs read. Building it here picked up the previous
# run's copies -- the same failure the note above describes for the triptych and
# hub sheets.
f06_cleanup <- function() {
...
message("F05 main panels complete")
