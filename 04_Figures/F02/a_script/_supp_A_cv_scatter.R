...
top_cv_labels <- bind_rows(
  scatter_df |> filter(age == "Young") |> slice_max(max_cv, n = 8, with_ties = FALSE),
...
r_old <- cor(scatter_df$cv_pre[scatter_df$age == "Old"],
  scatter_df$cv_post[scatter_df$age == "Old"],
  use = "complete.obs"
)
...
r_annotations <- tibble(
  ...
  label = c(
    sprintf("r = %.2f [%.2f, %.2f]", r_young, ci_young["lo"], ci_young["hi"]),
...
pA12 <- ggplot(scatter_df, aes(x = cv_pre, y = cv_post)) +
  ...
  geom_label_repel(
    ...
    size = FIG_AXIS_TEXT / .pt,
    label.padding = unit(1, "pt"),
    label.size = 0.3, max.overlaps = 20,
    # box.padding, because force alone let S100A9 settle under S100A8: repel
    # converged on a placement it scored as fine, with the two boxes touching.
    box.padding = unit(0.45, "lines"),
    xlim = c(0, axis_max_cv * 0.98), ylim = c(0, axis_max_cv * 0.90),
    force = 30, force_pull = 0.02, max.iter = 40000,
    segment.size = 0.2, segment.color = "grey50",
    min.segment.length = 0, seed = 42, show.legend = FALSE
  ) +
  geom_label(
    data = r_annotations, aes(label = label),
  ...
  labs(
    ...
    subtitle = sprintf(
      paste0(
        "Per-protein CV%% Pre vs Post — measurement reproducibility | ",
  ...
  theme(
    ...
    plot.subtitle = element_text(
      hjust = 0, size = FIG_SUBTITLE_SIZE - 1.0,
      face = "bold.italic", color = "grey40",
...
pA3 <- ggplot(delta_wide, aes(x = dcv_Young, y = dcv_Old)) +
  ...
  geom_label_repel(
    data = top_delta, aes(label = gene, fill = mean_dcv_capped),
    color = "white", fontface = "bold",
    size = FIG_AXIS_TEXT / .pt,
    label.padding = unit(1, "pt"), label.size = 0.3,
    max.overlaps = 25,
    xlim = c(-73, 222), ylim = c(-97, 180),
    segment.size = 0.2, segment.color = "grey50",
  ...
  scale_color_gradient2(
    low = HEATMAP_LO, mid = "grey95", high = HEATMAP_HI,
  ...
  scale_fill_gradient2(
    low = HEATMAP_LO, mid = "grey55", high = HEATMAP_HI,
    midpoint = 0, limits = c(-cv_cap, cv_cap),
    guide = "none"
  ) +
  coord_fixed(xlim = c(-75, 225), ylim = c(-100, 200)) +
  labs(
    x = expression(bold(Delta * "CV%"[Young])),
