...
build_scatter <- function(df, x_col, y_col, x_lab, y_lab, title, quad_labels, sig_col) {
  ...
  ggplot(df, aes(x = .data[[x_col]], y = .data[[y_col]])) +
    ...
    geom_label_repel(aes(label = bio_label),
      ...
      size = txt_lab, fontface = "bold", max.overlaps = Inf,
      segment.size = 0.3, segment.color = "grey40", min.segment.length = 0,
      box.padding = 0.8, point.padding = 0.6, force = 40, force_pull = 0.08,
      label.padding = unit(1.5, "pt"), label.r = unit(1.5, "pt"),
      label.size = 0, seed = 42, show.legend = FALSE) +
...
ggsave(file.path(RPT, "PILOT_panel_B_scatters.png"), scatters_panel,
...
ggsave(file.path(RPT, "PILOT_panel_B_legend.png"), p_legend,
       width = 90, height = 16, units = "mm", dpi = 300, limitsize = FALSE)

message("PILOT Panel B: scatters + legend saved separately")
