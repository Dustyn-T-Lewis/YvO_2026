...
RPT <- "04_Figures/F06/b_reports/pilot"
...
txt_cell  <- scale_text(BASE_GENE, PA_W) * 0.95 * 1.25 - 0.3
txt_count <- scale_text(BASE_COUNT, PA_W) * 0.85 * 1.25 - 0.3
txt_axis  <- scale_text(BASE_STAT, PA_W) * 1.5 * 1.25
txt_brack <- scale_text(BASE_GENE, PA_W) * 1.15 * 1.25

xmin_all <- min(xpos) - 0.55
xmax_all <- max(xpos) + 0.55

# Section shading: grey for contrasts, Young/Old tint for phenotype.
# Alternating alpha (0.08 / 0.18) strengthens block-to-block separation so the
# 7 column groups read as discrete bands (matches figure-style §7 background shading).
shading <- tibble(
...
# Brackets (7 sections — shorter labels for narrow 2-column sections)
brackets <- tribble(
  ~label,                              ~start,     ~end,
...
p_heat     <- build_heatmap(heat_df, col_labels, trait_order,
                             xmin_all, xmax_all, txt_cell, shading)
# Overlay thin grey rules between the 7 column groups (drawn after tiles so the
# borders sit above the fill but below the significance rings).
p_heat     <- p_heat +
...
# Save
# write_csv(heat_df, file.path(DAT, "01_panel_A_heatmap_data.csv"))  # skipped for pilot

ggsave(file.path(RPT, "PILOT_panel_A_heatmap.png"), fig_A,
       width = PA_W, height = PA_H, units = "mm",
       dpi = 300, limitsize = FALSE)

# Trim disabled for pilot — was cropping preservation bars
# if (requireNamespace("magick", quietly = TRUE)) {
#   .pA_path <- file.path(RPT, "PILOT_panel_A_heatmap.png")
#   magick::image_write(
...
# CI audit and c_data writes skipped for pilot
message("  Panel A pilot saved")
