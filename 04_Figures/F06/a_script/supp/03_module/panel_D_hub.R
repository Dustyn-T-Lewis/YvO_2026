...
#   5 key modules, individual output per module. Pathway hulls drawn directly
#   onto the network via ggforce::geom_mark_hull (no inset).
...
#   Generates: 03_module/SUPP_hub_{mod}_{slug}.png, c_data/04_panel_D_*.csv
################################################################################
...
# Jaccard 0.5: >50% gene overlap = redundant (cf. Supek 2011 REVIGO)
# padj 0.05: BH-corrected hypergeometric (fgsea::fora)
...
# --- Export individual panels (supp_F through supp_J within 03_module)
...
# --- Panel E: 3-network composite (shared layout) ---
...
# Attempt to collect shared legends via patchwork
# Keep first plot's legend for extraction
p_with_legend <- results[[KEY_MODULES[1]]]$plot
...
# removed 2026-04-09 — only key-module hubs are kept; brown was promoted
# to top-level (panel_D_hub_brown_SUPP) prior to removal.
