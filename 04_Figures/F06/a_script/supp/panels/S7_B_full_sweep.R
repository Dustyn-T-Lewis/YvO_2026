...
if (!file.exists(screen_src)) {
  source_panel("04_Figures/F06/a_script/main/panels/C_hero_grid.R")
}
...
screen_counts <- sprintf("Screen: %d tests; %d raw-sig, %d BH-sig.",
                         nrow(screen), sum(screen$p_raw < 0.05),
                         sum(screen$p_bh < 0.05))
...
# Named from the screen by the same rule the main panel uses, so this caption
# cannot drift away from the cells that are actually drawn there.
hero_names <- screen |>
  ...
  arrange(p_raw) |>
  slice_head(n = 6) |>
  # "vs" rather than an arrow: the quartz PDF device drops U+2192 silently,
  # and the main panel uses the same wording.
  mutate(label = sprintf("%s %s vs %s",
                         ifelse(source == "baseline", "baseline", "\u0394"),
                         vapply(module, pretty_mod, character(1)),
                         outcome_nice[outcome])) |>
  pull(label) |>
  paste(collapse = ", ")
