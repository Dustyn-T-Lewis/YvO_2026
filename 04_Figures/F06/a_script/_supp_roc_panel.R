#!/usr/bin/env Rscript
# F06 Supplementary — ROC panel (standalone SUPP)
#
...
clf_meta <- tibble::tribble(
  ...
  "Age|proteins",              "Age (subject)",     "Proteome (2106)",          "#D6604D",
...
composite <- composite +
  plot_annotation(
    title    = "F06 Supplementary \u2014 ROC menu: what is predictable, what is not",
    subtitle = paste0(
    ...
    caption = paste0(
      ...
      "dashed diagonal = chance. Pilot data: 04_Figures/F06/c_data/classifier_pilot/."),
    theme = theme(
...
W_in <- 11; H_in <- 11
ggsave(file.path(RPT_PDF, "SUPP_F06_roc_panel.pdf"), composite,
       width = W_in, height = H_in, units = "in",
       device = get_pdf_device())
ggsave(file.path(RPT_PNG, "SUPP_F06_roc_panel.png"), composite,
       width = W_in, height = H_in, units = "in", dpi = 300)

message("SUPP_F06_roc_panel written")
