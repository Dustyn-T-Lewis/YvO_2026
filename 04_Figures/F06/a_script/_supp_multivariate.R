...
# Writes: b_reports/supp/{png,pdf}/panels/SUPP_F06_multivariate_classifier.{png,pdf}
#
...
pG_roc <- ggplot(bar_df, aes(x = auc, y = display)) +
  ...
  theme(panel.grid.major.y = element_blank(),
        ...
        plot.title    = element_text(face = "bold",
                                     size = composite_text_sizes(PG_ROC_W, 178)$title,
                                     lineheight = 1.15,
                                     margin = margin(t = 10, b = 3, unit = "pt")),
        plot.subtitle = element_text(size = composite_text_sizes(PG_ROC_W, 178)$subtitle,
                                     face = "bold.italic", color = "grey30",
                                     lineheight = 1.2,
                                     margin = margin(t = 2, b = 8, unit = "pt")),
        plot.margin  = margin(t = 8, r = 1, b = 1, l = 2))

ggsave(file.path(RPT_PNG, "SUPP_F06_multivariate_classifier.png"), pG_roc,
       width = PG_ROC_W, height = PG_ROC_H, units = "mm", dpi = 300)
ggsave(file.path(RPT_PDF, "SUPP_F06_multivariate_classifier.pdf"), pG_roc,
       width = PG_ROC_W, height = PG_ROC_H, units = "mm",
       device = get_pdf_device())

message("  SUPP_F06_multivariate_classifier saved")
