...
pA_title    <- "WGCNA Module\u2013Trait Associations"
pA_subtitle <- paste0(length(mod_order), " modules | LMM (BH) | Stratified r (BH per-trait)")
# strip_for_composite() would propagate legend.position="none" to colorbar
# sub-plots; instead strip only title/subtitle/tag (colorbars stay embedded)
pA          <- fig_A + plot_annotation(title = NULL, subtitle = NULL)
