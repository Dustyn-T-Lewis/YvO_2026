# Shared Pattern Heatmap Panel — used by F04 (concordance) and F05 (reversal)
...
p <- p +
  coord_cartesian(xlim = c(0.0, X_BAR_MAX + 2.0),
                  ylim = c(BAR_YMAX + ROW_H * 7.5, -ROW_H * 0.05),
                  expand = FALSE) +
  theme(plot.margin = margin(2, -28, 6, -14, "mm"))

message(sprintf("%s Panel C (pattern heatmap) done", cfg$fig_id))
