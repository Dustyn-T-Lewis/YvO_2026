# Shared Panel D -- fry Rotation Test: Barcode + flanking ORA
# Requires: cfg list defined by caller before source()
#
...
# cfg fields:
#   fig_id            - "F04" or "F05"
#   contrast_source   - contrast whose DEPs define gene sets ("Training_Young" or "Aging")
#   contrast_test     - contrast tested by fry (always "Training_Old")
#   set_prefix        - prefix for set names ("ty" or "aging")
#   expected_up       - expected fry direction for source-Up set ("Up" or "Down")
#   expected_down     - expected fry direction for source-Down set ("Down" or "Up")
#   driving_up_sign   - sign filter for driving proteins in up set: "pos" or "neg"
#   driving_dn_sign   - sign filter for driving proteins in down set: "neg" or "pos"
#   up_title_fmt      - sprintf template for up barcode title (one %d for n)
#   dn_title_fmt      - sprintf template for down barcode title (two args: %d n, %s ns suffix)
#   fig_color         - color for barcode ES curves
#   stat_corner_up    - "topright" or "bottomleft"
#   stat_corner_dn    - "topright" or "bottomleft"
#   has_circularity   - TRUE if circularity diagnostic needed (F05)
#   ora_flank_up_label - ORA flanking plot title for up set
#   ora_flank_dn_label - ORA flanking plot title for down set
#   ora_supp_up_label  - supplementary ORA bar label for up set
#   ora_supp_dn_label  - supplementary ORA bar label for down set
#   ora_supp_title     - supplementary ORA composite title
#   ora_supp_subtitle  - supplementary ORA composite subtitle
#   label_map          - named char vector for explicit ORA label abbreviations
#   force_inside_labels - character vector of labels forced inside bars (or NULL)
#   long_label_mode    - "wrap" or "truncate" for long ORA labels
#   title              - composite plot title
#   subtitle_fmt       - sprintf template for subtitle (varies by circularity)
#   rpt_png, rpt_pdf, rpt_sup_png, rpt_sup_pdf, dat - output directories
#   panel_w            - panel width in mm (default 178)
# ---------------------------------------------------------------------------
...
# -- Step 4: Circularity diagnostic (F05 only) ---------------------------------
...
# -- Step 5: Define gene sets from source contrast ----------------------------
...
# Driving up: proteins in source-up set whose test-contrast t goes in expected direction
...
# Add circularity annotation if applicable
if (!is.null(circ_r)) {
  p_t <- p_t +
...
make_flanking_ora <- function(ora_df, set_label, bar_color,
  ...
  # Adaptive label placement: inside-centered when bar is wide enough,
  # outside-right when bar is too short
...
# Build subtitle from format string
...
# -- Supplementary: Leading-edge ORA bar chart ---------------------------------
...
if (!is.null(p_ora_up) || !is.null(p_ora_dn)) {
  ...
  message(sprintf("%s Panel D ORA (supp) done", cfg$fig_id))
}
...
message(sprintf("%s Panel D (fry) done", cfg$fig_id))
