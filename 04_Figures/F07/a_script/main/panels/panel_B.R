# Figure 7 — Panel C: Module × Phenotype coupling is age-dependent (2×3)
#
# Capstone redesign v2 2026-04-14: replaces forest plot with a 2×3 grid of
# hero scatters — one per strong age-flipped (or dual-significant) ΔME→Δpheno
# relationship. Each subpanel mirrors Panel C's hero styling (age-colored
# points, per-age regression + CI ribbon, per-stratum r/p annotation).
#
# The 6 picks are the strongest signed/age-interacting ΔME → Δphenotype
# associations (ranked by min within-age p across outcomes). They cover
# four distinct outcomes (ΔVL, ΔLBM, ΔT1, ΔT2) and four distinct modules
# (green, brown, turquoise, yellow + baseline brown + baseline yellow).
#
# Output:
#   MAIN_panel_B_hero_grid.png — 2×3 hero scatter grid (full-width bottom)
...
message("Panel D: 2x3 hero scatter grid (ΔME × Δpheno)...")
...
# Frozen list so the figure is reproducible even if ranking shifts slightly.
# Mix of ΔME and baseline-ME predictors; outcomes span muscle size, lean mass,
# and composition (T1/T2).
# Ordered by strongest within-age p (strongest first, for reading-order).
HERO_PICKS <- tribble(
  ...
  "delta_ME",  "MEyellow",     "delta_T2"    # dual-sig: Y *, O †
)
...
build_hero_mini <- function(x_source, module, outcome) {
  ...
  # In-plot stat box: stats placed in top-right corner of panel, stacked.
  # Y-axis is data-driven (not 0..1) so use grobs via annotation_custom-style
  # trick: place via geom_richtext at data-space coordinates computed from
  # data range. We use annotate with npc-like positions by using layer data.
  stat_label <- sprintf(
...
# Compute all module × source × outcome × stratum correlations to quantify
# the multiple-testing burden behind the hero picks.
