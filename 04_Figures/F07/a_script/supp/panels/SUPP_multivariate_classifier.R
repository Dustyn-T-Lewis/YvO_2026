...
#   3. ΔME:       training response (Post − Pre)
#   4. Combined:  mean(Pre, Post) per subject — AVERAGED not stacked
#
# Why averaged (not stacked): Stacking 60 obs (30 subj × 2 TP) introduces
# within-subject correlation that inflates effective N and overfits with
# elastic net. Averaging collapses each subject to a single "time-invariant"
# eigengene vector that preserves the age signal while removing training-
# induced within-subject variance. Empirically: Combined AUC=0.920,
# perm p=0.044 vs Pre-only 0.818 p=0.08.
#
# All use top-k feature selection + logistic regression with nested LOOCV
# for k optimization (k=2..5). Feature selection inside each fold
# (Ambroise & McLachlan 2002, PNAS 99:6562). Elastic net tested but
# overfits with N=30 (null AUC~0.86). Permutation test on best classifier.
#
# CROSS-REFERENCE: PLIER (F06/supp/plier/) performs the same LOOCV age
# classification using PLIER LV scores instead of WGCNA MEs.
...
# Compute ΔME from aligned pre/post (guaranteed 30 subjects)
...
# Elastic net overfits with N=30 (null AUC~0.86); top-k + GLM is appropriate.
# k optimized via nested inner LOOCV (k=2..5) inside each outer fold.
# ─────────────────────────────────────────────────────────────────────────────
run_topk_loocv <- function(labels, me_data, k_range = 2:5) {
  ...
  for (i in seq_len(n)) {
    ...
    # Feature ranking by |cor| with outcome in training fold
...
message("  Running ΔME classifier...")
...
# Feature stability per classifier
...
# Fast LOOCV for permutations: fixed k (no nested CV), ~100× faster
...
# Select data and median k for best classifier
...
# ─────────────────────────────────────────────────────────────────────────────
# Visualization: Stratified-AUC bars with text inside bars + per-bar ROC
# sparklines at each bar's end (dynamic x-position).
# 2026-04-14 refresh:
#   - Classifier name + top-2 LOOCV-selected features printed INSIDE each bar
#   - Sparklines sit immediately to the right of each bar end (follow length)
#   - No CI error bars, no ★/best highlight — keep panel minimal
# ─────────────────────────────────────────────────────────────────────────────
PG_ROC_W <- 580
PG_ROC_H <- 553
txt_annot <- scale_text(BASE_STAT, PG_ROC_W)

# Top-2 feature annotation per classifier from feature_freq
...
# Per-classifier ROC curves for sparklines
...
# Sparkline inset: minimal ticks (0 bottom-left; 1 at top-y & right-x),
# small axis labels flush to axis, wider footprint.
...
# Factor-level → plot-y mapping (last level = top of plot)
disp_levels <- levels(bar_df$display)
bar_y <- setNames(seq_along(disp_levels), disp_levels)

# Sparklines: xmin anchored to AUC-label position (just past bar end + number);
# each sparkline is the same fixed width so they stack in a vertical column
# aesthetically while still being tied to their bar's y-position.
spk_width  <- 0.55
spk_half_h <- 0.48
spk_xmax   <- 1.55
...
pG_roc <- ggplot(bar_df, aes(x = auc, y = display)) +
  ...
  # Stacked top-2 features inside each bar (bigger size, centered)
