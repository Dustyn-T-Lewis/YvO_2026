...
#
# Purpose: address the lit-review concern that in-sample AUCs (computed on the
# same subjects whose module eigengenes were derived from the same cohort) are
# optimism-biased. For each top-12 (row, module) pair, leave one subject out,
# refit the module's 1st PC on n-1 training subjects, project the held-out
# subject's protein profile onto that PC, and compute an out-of-fold AUC.
#
# Limitation: WGCNA module assignments are held fixed at their full-sample
# definitions. This script does NOT refit the WGCNA network per fold. It
# therefore addresses eigengene-projection optimism but not module-definition
# circularity. A full WGCNA-refit LOSO would be the stronger sensitivity
# (~15 min runtime) — flagged for future runs if this partial LOSO shows
# meaningful drops.
#
# Reads:
#   04_Figures/F06/c_data/datExpr.rds            — samples × proteins (binary RDS)
#   04_Figures/F06/c_data/module_colors.rds      — protein → module assignments (binary RDS)
#   04_Figures/F06/c_data/me_pre.rds             — subject × module MEs (Pre, binary RDS)
#   04_Figures/F06/c_data/me_post.rds            — subject × module MEs (Post, binary RDS)
#   04_Figures/F06/c_data/F06_supplementary.xlsx — sheets metadata_subj_age + metadata_pheno_wide
#   04_Figures/F07/c_data/module_grid/module_grid_summary.csv  — in-sample AUCs
#
# Writes:
...
datExpr   <- readRDS("04_Figures/F06/c_data/datExpr.rds")  # samples × proteins
mod_cols  <- readRDS("04_Figures/F06/c_data/module_colors.rds")  # protein → colour
...
# In-sample AUCs: prefer CSV (if SUPP_module_grid.R just ran), else fall back
# to the F07_supplementary.xlsx sheet (persisted after cleanup_after_workbook).
in_sample_csv <- "04_Figures/F07/c_data/module_grid/module_grid_summary.csv"
in_sample_xlsx <- "04_Figures/F07/c_data/F07_supplementary.xlsx"
...
# Subject key from WGCNA sample IDs: S01_Pre → S01
sample_ids   <- rownames(datExpr)                                # e.g. S01_Pre / S01_Post
...
# ---- Outcome vectors (mirror SUPP_module_grid.R) ---------------------------
...
# Compute PC1 on training rows, return held-out projection.
# Mirrors WGCNA::moduleEigengenes PC1 convention: scale proteins, SVD,
# first right-singular vector as loadings, sign-align by mean correlation.
loso_me <- function(full_mat, train_rows, holdout_rows) {
  ...
  # Sign-align so training ME correlates positively with mean protein abundance
...
# Get module-specific protein columns from datExpr (samples × proteins).
# module_colors.rds is a positional vector aligned to colnames(datExpr) — no names.
stopifnot(length(mod_cols) == ncol(datExpr))
module_proteins <- function(module) colnames(datExpr)[mod_cols == module]

# Build subject × protein matrices for Pre, Post, Combined
...
for (i in seq_len(nrow(top12))) {
  ...
  # Decide which subject-level design + predictor to use per row
  if (row == "Age") {
  ...
  if (paired) {
    ...
    # Pooled LOSO: hold out one subject (both tp); fit PC on remaining 2*(n-1) samples
