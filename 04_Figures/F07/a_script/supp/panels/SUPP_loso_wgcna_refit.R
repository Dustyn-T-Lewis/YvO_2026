...
#
# Purpose: strongest sensitivity for module-definition circularity. For each of
# 32 subjects, remove their 2 samples from datExpr, refit the WGCNA network on
# the remaining 60 samples (same parameters as YvO_WGCNA_run.R, soft power
# fixed at 12), match training modules to full-sample modules by Jaccard
# overlap of protein assignments, project held-out subject's module eigengenes
# onto training PCs, and compute a held-out AUC per (row, module) pair.
#
# Companion to SUPP_loso_sensitivity.R (eigengene-projection LOSO with fixed
# modules). This script additionally tests whether the MODULE DEFINITIONS are
# stable under LOSO — i.e. whether top-12 AUCs survive module redefinition.
#
# Runtime: ~13–15 min (32 folds × ~25 s/fold blockwiseModules).
#
# Reads:
#   04_Figures/F06/c_data/{datExpr,module_colors,me_pre,me_post}.rds  (binary)
#   04_Figures/F06/c_data/F06_supplementary.xlsx  (metadata_subj_age + metadata_pheno_wide sheets)
#   04_Figures/F07/c_data/F07_supplementary.xlsx  (module_grid_summary sheet)
#
# Writes:
...
datExpr   <- readRDS("04_Figures/F06/c_data/datExpr.rds")     # 62 samples × 2113 proteins
...
names(mod_full) <- colnames(datExpr)                          # name for lookup
...
# ---- Outcome vectors (matching SUPP_module_grid.R) --------------------------
...
# ---- Helpers ----------------------------------------------------------------
# Jaccard-match training modules to full-sample modules
match_modules <- function(mod_train, mod_full) {
  ...
  # Greedy best match per full-sample module
...
# Compute training MEs + loadings per module; return ME predictions for any rows
# of datExpr. Module assignments are the training-fold colors.
compute_me_projection <- function(datExpr_full, train_rows, mod_train_full_len,
  ...
  # Sign-align so ME correlates positively with mean protein abundance (WGCNA convention)
...
# Block per-fold projection for all 8 MODULES at once
...
# Helper to extract predicted ME per subject per module/timepoint
...
for (i in seq_len(nrow(top12))) {
  ...
  # Pivot Pre/Post into wide for subject-level rows; Combined = (Pre + Post)/2
...
# Module stability summary (Jaccard across all folds for each full-sample module)
